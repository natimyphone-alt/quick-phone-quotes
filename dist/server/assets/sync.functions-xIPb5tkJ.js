import { T as TSS_SERVER_FUNCTION, a as createServerFn } from "./server-CmJRbabY.js";
import { r as requireSupabaseAuth } from "./auth-middleware-D8JA3jUC.js";
import { z } from "zod";
...
function parsearResultadosPC(html) {
  const productos = [];
...
${html.slice(0, 3e3)}`);
    ...
    return {
      ok: false,
      productos: [],
      message: e?.message || "Error buscando en Patagonia Cell"
    };
  }
});
...
const syncPatagonia = createServerFn({
  ...
  return {
    ...
    updated: 0,
    errors: 0,
    message: "Patagonia Cell usa búsqueda en tiempo real. No requiere sincronización previa."
  };
});
...
const syncTodo = createServerFn({
  ...
  return [{
    ...
    updated: 0,
    errors: 0,
    message: "Búsqueda en tiempo real, no requiere sincronización."
  }];
});
export {
